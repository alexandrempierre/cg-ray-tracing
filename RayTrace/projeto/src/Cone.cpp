...
Intersection Cone::Intercepta(const Raio& r_vis, IntersectionMode mode, float threshold)
{
  ...
  float t1, t2; // esticamentos (t*r_vis) do raio que interceptam o cone
  Ponto_3D P1, P2; // Pontos em que o raio intercepta o cone (podem ser coincidentes)
  ...
  a = direcao.produtoEscalar(direcao) + r_vis.Y0()*r_vis.Y0()/(altura*altura);
  b = 2*(direcao.produtoEscalar(K) - raio*raio*r_vis.Y0()/altura);
  ...
  if (delta >= 0) {
      ...
      // Verifica se pelo menos um dos pontos está dentro do cone
